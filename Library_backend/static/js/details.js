  ...
  fetch('/api/recent-books/')
    ...
    .then(data => {
      if (data.success && data.books.length > 0) {
        recentBooksContainer.innerHTML = data.books.map((book) => `
        ...
        recentBooksContainer.innerHTML = '<div class="empty-state">No books added from this page yet. Your next saved book will appear here.</div>';
      }
    })
...
form.addEventListener("submit", (event) => {
  ...
  const bookData = buildBookFromForm();
